...
impl CapabilityCharacter {
    pub(crate) fn from_pdfium(character: &PdfPageTextChar<'_>) -> Self {
...
impl FontCapability {
    pub(crate) fn from_pdfium(font: &PdfFont<'_>) -> Self {
        ...
        Self { name: font.name(), embedded, embedded_error, data_length, data_error }
    }
}
...
impl ObjectCapability {
    pub(crate) fn from_pdfium(object: &PdfPageObject<'_>, next_paint_order: &mut usize) -> Self {
        ...
        *next_paint_order += 1;
        ...
        let (kind, children) = match object {
            ...
            PdfPageObject::XObjectForm(form) => {
                ("form", form.iter().map(|object| Self::from_pdfium(&object, next_paint_order)).collect())
            }
        };

        Self { paint_order, kind, active, active_error, children }
    }
}
...
impl TextObjectRenderingCapability {
    pub(crate) fn inspect(page: &PdfPage<'_>) -> Self {
        ...
        let mut text_objects: Vec<_> =
            page.objects().iter().filter(|object| object.as_text_object().is_some()).collect();
...
impl FontMappingCapability {
    pub(crate) fn for_pdfium_render_0_9_3() -> Self {
        Self {
...
mod tests {
    ...
    fn serializes_owned_diagnostic_values() {
        ...
        assert_eq!(json["pages"][0]["text_object_rendering"]["suppression_changed_bitmap"], true);
